...
for kfold = 1:K
    ...
    valind = randomind((kfold-1)*hop + 1:hop*kfold); % ...Select validation indices
    ...
    for klam = 1:Nlam
        ...
        SEval(kfold,klam) = inv(Nval)*norm(t(valind)-X(valind,:)*what)^2; % ...Calculate validation error for this estimate
        SEest(kfold,klam) = inv(N-Nval)*norm(t(estind)-X(estind,:)*what)^2; % ...Calculate estimation error for this estimate
...
[Y,I] = min(MSEval); % ...Select optimal lambda 
