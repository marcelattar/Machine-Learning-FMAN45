...
Nval = floor(N/K);
cvhop = Nval;
randomind = randperm(framehop);% ...Select random indices for picking out validation and estimation indices. 
...
for kframe = 1:Nframes % First loop over frames
    ...
    for kfold = 1:K % Then loop over the folds
        ...
        valind = randomind((kfold-1)*cvhop + 1:cvhop*kfold); % ...Select validation indices
        estind = randomind; % ...Select estimation indices
        estind((kfold-1)*cvhop + 1:cvhop*kfold)=[];
        
        %valind = randomind(cvlocation+1:cvlocation+Nval); % Select validation indices
        %estind = [randomind(1:cvlocation), randomind(cvlocation+Nval+1:length(randomind))]; % Select estimation indices
        ...
        for klam = 1:Nlam  % Finally loop over the lambda grid
            ...
            SEval(kfold,klam) = SEval(kfold,klam) +  (1/Nval)*norm(t(valind)-X(valind,:)*what)^2; % ...Add validation error at current frame, fold and lambda to the validation error for this fold and lambda, summing tthe error over the frames
...
for kframe = 1:Nframes
    ...
    Wopt(:,kframe) = lasso_ccd(t,X,lambdaopt,wold); % ... Select Wopt
    framelocation = framelocation + framehop;
end
