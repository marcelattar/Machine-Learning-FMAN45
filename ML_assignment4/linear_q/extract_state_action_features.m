...
for action = 1 : 3 % Evaluate all the different actions (left, forward, right).
    ...
    state_action_feats(1, action) = (norm(apple - prev_head_loc) - ...
                                    norm(apple - next_head_loc))/sqrt(2*(N-2)^2);
    
    % Feature 1. Does the action bring us closer to the apple (Binary)?
%     if (norm(apple - prev_head_loc) - norm(apple - next_head_loc)) > 0
...
%     end
    ...
    % Feature 3. Will this action lead to us eating the apple?
    if (grid(next_head_loc(1),next_head_loc(2)) == -1)
    ...
    % Feature 3. How "grouped up" is the snake? 
%     cropped_grid = grid(2:N-1,2:N-1);                              
%     indices = find(cropped_grid == 1);
%     [I, J]  = ind2sub(size(cropped_grid),indices);
%     sn_pos = [I,J];
%     sn_pos = [sn_pos;next_head_loc];
%     
%     state_action_feats(3, action) = max(pdist(sn_pos))/sqrt(2*(N-2)^2);
