...
while 1
    ...
    while 1
        ...
        for state_idx = 1 : nbr_states
            ...
            % Checking the action for a given state
            action = policy(state_idx);
            
            % Checking what state that action will take us to
            next_index = next_state_idxs(state_idx,action);
            
            % Updating the values
            if next_index == -1 % This is when we eat an apple
                values(state_idx) = rewards.apple + 0;
            elseif next_index == 0 % We died
                values(state_idx) = rewards.death + 0;
            else % We lived
               values(state_idx) = rewards.default + gamm*values(next_index);
            end
    ...
    for state_idx = 1 : nbr_states
        ...
        % Updating the values
            if next_index == -1 % This is when we eat an apple
                temp_values(i) = rewards.apple + 0;
            elseif next_index == 0 % We died
                temp_values(i) = rewards.death + 0;
            else % We lived
        ...
        if policy_stable
            policy_stable = (old_action == policy(state_idx));
        end
    end
