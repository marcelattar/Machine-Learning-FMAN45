...
rewards = struct('default', 0, 'apple', 10, 'death', -1); % Reward signal
gamm    = 0.9;  % Discount factor in Q-learning
alph    = 0.50; % Learning rate in Q-learning (automatically set to zero during testing)
eps     = 0.1; % Random action selection probability in epsilon-greedy Q-learning (automatically set to zero during testing)
...
for i = 1 : nbr_ep
    ...
    while 1
        ...
        % can be rewritten as Q(s,a) <-- Q(s,a) + alpha * (sample - Q(s,a))
        sample                    = reward + ...
                                      gamm*max(Q_vals(next_state_idx,:)); 
        pred                      = Q_vals(state_idx,action);  
        td_err                    = sample - pred; 
        Q_vals(state_idx, action) = Q_vals(state_idx, action) ...
                                            + alph*td_err; 
