function cifar10_starter()
    ...
    % Increase the nbr of filters to 21 from 16
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,3,21)/sqrt(5*5*3/2), 'biases', zeros(21,1)),...
        'padding', [2 2]);
    ...
    % Added three extra layers here -------------------
    
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', randn(5,5,21,21)/sqrt(5*5*21/2), 'biases', zeros(21,1)),...
        'padding', [2 2]);
    ...
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,21,16)/sqrt(3*3*21/2), 'biases', zeros(16,1)),...
        'padding', [2 2]);
    
    net.layers{end+1} = struct('type', 'relu');
    
    % You have to change the size of randn depending on nbr of layers and
    % size of filters.
    ...
    % 1. I changed the learning_rate to 1e-2 from 1e-3. 
    % 2. I changed iterations from 5000 to 10 000
    training_opts = struct('learning_rate', 1e-3,...
        'iterations', 10000,...
        'batch_size', 16,...
        'momentum', 0.95,...
        'weight_decay', 0.005); % This was 0.001 in begining
